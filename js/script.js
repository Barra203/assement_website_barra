$(document).ready(function () {
            ...
            if (!itemQuantity[formattedItemName]) {
                ...
                var newElement = `
                    <div class="barang">
                        <div class="detail-kiri">
                            <p>${itemName}</p>
                            <p class="normal">Price: Rp. ${itemPrice.toLocaleString('id-ID')}</p>
                        </div>
                        <div class="detail-kanan">
                            <p style="text-align: right; font-size: small;">${itemPriceText}</p>
                            <p class="normal" style="display: flex; gap: 1em;">
                ...
                $('.belanja .barang').each(function () {
                    ...
                    if (currentItemName === itemName) {
                        $(this).find('.detail-kiri .normal').text('Price: Rp. ' + updatedItemPrice.toLocaleString('id-ID'));
                    }
                });
                updateTotalAfterQuantityChange();
            }
    ...
    $('.belanja').on('click', '.remove', function () {
        ...
        var itemPrice = parseFloat(itemPriceText.replace('Price: Rp. ', '').replace('.', '').replace(',', '.'));
        ...
        if (!isNaN(itemPrice)) {
            ...
            if (currentQuantity > 1) {
                ...
                $item.find('.detail-kiri .normal').text('Price: Rp. ' + updatedItemPrice.toLocaleString('id-ID'));
                updateTotalAfterQuantityChange();
            } else {
